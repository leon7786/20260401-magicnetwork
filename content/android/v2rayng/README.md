<!-- i18n-explicit-blocks -->

### [zh-Hans]

# v2rayNG

v2rayNG 是 Android 上经典的 V2Ray/Xray 客户端，配置方式成熟。

- GitHub: <a href="https://github.com/2dust/v2rayNG" target="_blank" rel="noopener">https://github.com/2dust/v2rayNG</a>
- Latest Release: <a href="https://github.com/2dust/v2rayNG/releases/latest" target="_blank" rel="noopener">https://github.com/2dust/v2rayNG/releases/latest</a>
<a class="md-button md-button--primary download-latest" data-repo="2dust/v2rayNG" data-match="v2rayNG_.*_arm64-v8a\.apk$" data-fallback="https://github.com/2dust/v2rayNG/releases/latest">Android *arm64 APK</a>

### [zh-Hant-TW]

# v2rayNG

v2rayNG 是 Android 上經典的 V2Ray/Xray 客戶端，設定方式成熟。

- GitHub: <a href="https://github.com/2dust/v2rayNG" target="_blank" rel="noopener">https://github.com/2dust/v2rayNG</a>
- Latest Release: <a href="https://github.com/2dust/v2rayNG/releases/latest" target="_blank" rel="noopener">https://github.com/2dust/v2rayNG/releases/latest</a>
<a class="md-button md-button--primary download-latest" data-repo="2dust/v2rayNG" data-match="v2rayNG_.*_arm64-v8a\.apk$" data-fallback="https://github.com/2dust/v2rayNG/releases/latest">Android *arm64 APK</a>

### [en]

# v2rayNG


v2rayNG is a classic Android client for V2Ray/Xray with a mature configuration workflow.

- GitHub: <a href="https://github.com/2dust/v2rayNG" target="_blank" rel="noopener">https://github.com/2dust/v2rayNG</a>
- Latest Release: <a href="https://github.com/2dust/v2rayNG/releases/latest" target="_blank" rel="noopener">https://github.com/2dust/v2rayNG/releases/latest</a>
<a class="md-button md-button--primary download-latest" data-repo="2dust/v2rayNG" data-match="v2rayNG_.*_arm64-v8a\.apk$" data-fallback="https://github.com/2dust/v2rayNG/releases/latest">Android *arm64 APK</a>
